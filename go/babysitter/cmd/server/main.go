...
type MessageRouter struct {
	channels map[chan babysitter.Signal]struct{}
}

func NewMessageRouter() *MessageRouter {
	return &MessageRouter{
		channels: make(map[chan babysitter.Signal]struct{}),
	}
}

func (r *MessageRouter) Signal(sig babysitter.Signal) {
...
func (r *MessageRouter) OnSignal() chan babysitter.Signal {
	newCh := make(chan babysitter.Signal, 100)
	r.channels[newCh] = struct{}{}
	return newCh
}

func (r *MessageRouter) UnregisterChannel(ch chan babysitter.Signal) {
...
func handleSignal(router *MessageRouter) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		cmd := r.FormValue("cmd")
		cwd := r.FormValue("cwd")
		router.Signal(babysitter.Signal{
			Type: t,
			Key:  key,
			ID:   id,
			Cmd:  cmd,
